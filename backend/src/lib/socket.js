...
io.on('connection', (socket) => {
  ...
  io.on('connection', (socket) => {
    ...
    if (!userId) {
      console.warn(
        `[Socket.io] Connection rejected: Missing userId for socket ${socket.id}`,
      )
        ...
        if (res.modifiedCount > 0) {
          ...
          senders.forEach((senderIdStr) => {
            ...
            senderSockets.forEach((s) =>
              io.to(s).emit('messagesDelivered', { receiverId: userId }),
            )
          })
        }
      })
    ...
    socket.on('typing', async ({ receiverId }) => {
      ...
      receiverSockets.forEach((s) =>
        io.to(s).emit('userTyping', { senderId: userId }),
      )
    })
    ...
    socket.on('stopTyping', async ({ receiverId }) => {
      ...
      const receiverSockets = getReceiverSocketIds(receiverId)
      receiverSockets.forEach((s) =>
        io.to(s).emit('userStoppedTyping', { senderId: userId }),
      )
    })
