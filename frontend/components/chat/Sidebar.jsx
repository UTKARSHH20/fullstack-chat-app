      ...
      if (wentOffline.length > 0) {
        ...
        useChatStore.setState((state) => ({
          users: state.users.map((u) =>
            wentOffline.includes(u._id) ? { ...u, lastSeen: now } : u,
          ),
          selectedUser:
            state.selectedUser && wentOffline.includes(state.selectedUser._id)
        ...
        {['All', 'Unread', 'Work', 'Friends', 'Archived'].map((folder) => (
          <button
            ...
            className={`btn btn-xs ${
              selectedFolder === folder ? 'btn-primary' : 'btn-outline'
            }`}
          >
            {folder}
          </button>
      ...
      <div className="px-3 py-2 border-b border-base-200">
        <div className="stats stats-vertical shadow w-full">
          <div className="stat py-2">
            <div className="stat-title text-xs">Active Chats</div>
            <div className="stat-value text-lg">{activeChats}</div>
          </div>

          <div className="stat py-2">
            <div className="stat-title text-xs">Unread Messages</div>
            <div className="stat-value text-lg">{totalUnread}</div>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        {isUsersLoading ? (
          ...
          filtered.map((user) => {
            ...
            const folder =
              user.folder ||
              (user.name.charCodeAt(0) % 2 === 0 ? 'Work' : 'Friends')
            const preview = lm
              ? lm.message ||
                (lm.audio ? '🎤 Voice' : lm.image ? '📷 Image' : '')
              : ''
            return (
              <button
                key={user._id}
            ...
            ;<div className="flex items-center justify-between">
              {folder === 'Archived' ? (
                ...
                <div className="flex items-center gap-1">
                  ...
                  />
                  <p
                    className={`text-xs ${isOnline ? 'text-success' : 'text-base-content/40'}`}
                  >
                    {isOnline
                      ? 'Active now'
                      : user.lastSeen
