...
const formatLastSeen = (dateString) => {
  ...
  const isYesterday =
    new Date(nowCopy.setDate(nowCopy.getDate() - 1)).toDateString() ===
    date.toDateString();
...
export default function ChatWindow({ selectedUser, onBack, isMobileHidden }) {
  ...
  const pendingBookmarkTarget = useBookmarkStore(
    (state) => state.pendingBookmarkTarget,
  );
  const clearPendingBookmarkTarget = useBookmarkStore(
    (state) => state.clearPendingBookmarkTarget,
  );
  ...
  const scrollToMessage = (msgId) => {
    ...
    if (msgElement) {
      ...
      toast.error(
        "Message is not loaded in current view. Scroll up to load older messages.",
      );
    }
  };
  ...
  useEffect(() => {
    if (!pendingBookmarkTarget || !selectedUser || messages.length === 0)
      return;
  ...
  useEffect(() => {
    if (selectedUser?._id && messages.length > 0) {
      const hasUnseen = messages.some(
        (m) => m.senderId === selectedUser._id && m.status !== "seen",
      );
    ...
    const loadSuggestions = async () => {
      ...
      try {
        const res = await axiosInstance.get(
          `/messages/suggestions/${lastMessage._id}`,
        );
  ...
  const handleTyping = (e) => {
    ...
    setShowSpamWarning(
      suspiciousWords.some((word) => value.toLowerCase().includes(word)),
    );
  ...
  const receivedMessages = totalMessages - myMessages;

  const mediaMessages = messages.filter((msg) => msg.image || msg.audio).length;
  ...
  if (!selectedUser)
    return (
      <>
        <div
          className={`${isMobileHidden ? "hidden md:flex" : "flex"} flex-1 flex-col items-center justify-center bg-base-100 transition-colors duration-300`}
        >
          <div className="w-full h-full flex-1 text-base-content flex flex-col items-center justify-start py-6 px-6 font-sans antialiased overflow-y-auto selection:bg-primary/20">
            ...
            <div className="w-full max-w-2xl flex flex-col items-center text-center my-auto py-4">
              ...
              <div className="relative mb-8 flex items-center justify-center">
                <div className="absolute w-28 h-28 bg-primary/4 rounded-full blur-2xl"></div>
                <div className="border border-base-content/10 bg-base-content/2 p-4 rounded-full backdrop-blur-sm">
              ...
              <div className="space-y-4 mb-12">
                ...
                <p className="text-base-content/60 text-xs md:text-sm max-w-md mx-auto pt-2 leading-relaxed">
                  A quiet place to think out loud, draft something good, or
  ...
  return (
    <div
      className={`${isMobileHidden ? "hidden md:flex" : "flex"} flex-1 flex-col bg-base-100 min-w-0 h-full overflow-hidden`}
    >
      <div className="flex items-center gap-3 px-4 py-3 border-b border-base-200 shadow-sm shrink-0 sticky top-0 z-10 bg-base-100">
        ...
        <div>
          ...
          <p
            className={`text-xs ${isOnline ? "text-success font-medium" : "text-base-content/70"}`}
          >
            {typingUsers.includes(selectedUser._id) ? (
              <span className="text-success font-bold animate-pulse inline-block">
                typing...
              </span>
            ) : isOnline ? (
              "Online"
        ...
        <div className="ml-auto flex items-center gap-1">
          ...
          <button
            onClick={() =>
              startOutgoingCall(selectedUser._id, selectedUser.name, "audio")
            }
          ...
          <button
            onClick={() =>
              startOutgoingCall(selectedUser._id, selectedUser.name, "video")
            }
            className="btn btn-ghost btn-circle btn-sm text-base-content/70 hover:text-primary transition-colors"
            title="Video Call"
          >
          ...
          <button
            ...
            className={`btn btn-ghost btn-circle btn-sm ${
              showGallery ? "text-primary" : "text-base-content/70"
            }`}
            title="Media Gallery"
          >
      ...
      <div
        ...
        className="flex-1 overflow-y-auto overflow-x-hidden px-4 py-4 space-y-1 overscroll-contain"
      >
        {showNotes && (
          <div className="border-b border-base-200 p-3 bg-base-200">
            <div className="flex justify-between items-center mb-2">
              <h3 className="font-semibold text-sm">Collaborative Notes</h3>
      ...
      <ContextMenu
        ...
        isBookmarked={bookmarks.some(
          (item) => item.id === contextMenu.message?._id,
        )}
      />
      ...
      {showSpamWarning && (
        <div className="mx-3 mb-2 alert alert-warning py-2 text-sm">
          ⚠️ This message may contain potentially harmful or spam-related
          content.
        </div>
      )}
        ...
        {!isRecording &&
          (text.trim() || imageBase64 || audioBase64 || sending ? (
            ...
            <button
              ...
              className="btn btn-primary btn-sm btn-square shrink-0 rounded-full"
            >
              <Mic className="w-4 h-4" />
            </button>
          ))}
